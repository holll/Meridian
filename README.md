...
| `GEOLITE_DB_DIR` | 二进制同目录 | IP 归属数据库目录。启动时加载 `GeoLite2-Country.mmdb` + `GeoLite2-ASN.mmdb`（自动从 `github.com/P3TERX/GeoLite.mmdb` 镜像下载）；日志/分析页展示归属国家与运营商（中文标签：中国电信/中国联通/中国移动） |
